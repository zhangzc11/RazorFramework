...
int main ( ){
  std::ifstream ifs ( "list_DM_MonoB.list", std::ifstream::in );
  std::string fname;
  ...
  if ( ifs.is_open() )
      ...
      while ( ifs.good() )
	{
	  ...
	  //Filling Map Struct
	  Rmap[aux.mass].Lambda20 = FindLambda( 0.20, h_qtr_sel, h_qtr_gen )/g_c;
	  Rmap[aux.mass].Lambda40 = FindLambda( 0.40, h_qtr_sel, h_qtr_gen )/g_c;
	  Rmap[aux.mass].Lambda60 = FindLambda( 0.60, h_qtr_sel, h_qtr_gen )/g_c;
	  Rmap[aux.mass].Lambda80 = FindLambda( 0.80, h_qtr_sel, h_qtr_gen )/g_c;
  ...
  TFile* f3 = new TFile("/Users/cmorgoth/Software/git/DM_Limits/btagLimits/LimitSI.root");
  ...
  mg0->Draw("af");
  mg0->GetYaxis()->SetRangeUser(1, 2.0e3);
  ...
  //TLatex* tex = new TLatex(0.55,0.82,"g_{eff} = 2#pi");
  ...
  leg->AddEntry("NULL", "Razor-b limit: Scalar EFT operator, g_{eff} = 4#pi", "h");
  ...
  TLegend* leg2 = new TLegend(0.71,0.5,0.89,0.8,NULL,"brNDC");//(xmin, ymin, xmax, ymax)
  /*
  leg2->AddEntry(g_v_r80, "R_{2#pi#Lambda} = 80%" ,"l");
  leg2->AddEntry(g_v_r60, "R_{2#pi#Lambda} = 60%" ,"l");
  leg2->AddEntry(g_v_r40, "R_{2#pi#Lambda} = 40%" ,"l");
  leg2->AddEntry(g_v_r20, "R_{2#pi#Lambda} = 20%" ,"l");
  */
  ...
  */
  ...
  c->SaveAs("PLOTS/test_MonoB_Lambda4Pi.pdf");
  c->SaveAs("PLOTS/test_MonoB_Lambda4Pi.C");
