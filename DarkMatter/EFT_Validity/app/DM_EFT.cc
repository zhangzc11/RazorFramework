/*
Computes lambda cutoff scale limit for the same coupling (g),
but using different fractions for R_lambda = 0.2, 0.4, 0.6, 0.8
*/
//C++ INCLUDES
#include <iostream>
#include <string>
...
const float g_c = 1.0;
const float max_y = g_c*5.0*1e3;

int main ( ){
  /*
  TFile* ft = new TFile( "data/DMm1V_TOTAL.root" );
  TH1F* h_qtr_gent = (TH1F*)ft->Get( "h_entries" );
  TTree* treet = (TTree*)ft->Get( "outTree" );
  outTree* outt = new outTree( treet );
  TH1F* h_qtr_selt = outt->Loop( );
  std::cout << FindLambda( 0.60, h_qtr_selt, h_qtr_gent ) << std::endl;
  */
  std::ifstream ifs ( "dm_eft_validity_total.list", std::ifstream::in );
  std::string fname;
  ...
  if ( ifs.is_open() )
      ...
      while ( ifs.good() )
	{
	  ...
	  //Filling Map Struct
	  Rmap[map_key].Lambda20 = FindLambda( 0.20, h_qtr_sel, h_qtr_gen )/g_c;
	  Rmap[map_key].Lambda40 = FindLambda( 0.40, h_qtr_sel, h_qtr_gen )/g_c;
	  Rmap[map_key].Lambda60 = FindLambda( 0.60, h_qtr_sel, h_qtr_gen )/g_c;
	  Rmap[map_key].Lambda80 = FindLambda( 0.80, h_qtr_sel, h_qtr_gen )/g_c;
  ...
  TFile* f3 = new TFile("/Users/cmorgoth/Software/git/DM_Limits/SD_Graphs.root");
  ...
  TLegend* leg2 = new TLegend(0.68,0.5,0.88,0.8,NULL,"brNDC");//(xmin, ymin, xmax, ymax)
  
  leg2->AddEntry(g_av_r80, "R_{#Lambda} = 80%" ,"l");
  leg2->AddEntry(g_av_r60, "R_{#Lambda} = 60%" ,"l");
  leg2->AddEntry(g_av_r40, "R_{#Lambda} = 40%" ,"l");
  leg2->AddEntry(g_av_r20, "R_{#Lambda} = 20%" ,"l");
  ...
  */
  ...
  c->SaveAs("test_av_Lambda.pdf");
  c->SaveAs("test_av_Lambda.C");
  ...
  */
  ...
  f3 = new TFile("/Users/cmorgoth/Software/git/DM_Limits/SI_Graphs.root");
  ...
  leg2->AddEntry(g_v_r80, "R_{#Lambda} = 80%" ,"l");
  leg2->AddEntry(g_v_r60, "R_{#Lambda} = 60%" ,"l");
  leg2->AddEntry(g_v_r40, "R_{#Lambda} = 40%" ,"l");
  leg2->AddEntry(g_v_r20, "R_{#Lambda} = 20%" ,"l");
  
  /*
  leg2->AddEntry(g_v_r80, "R_{4#pi#Lambda} = 80%" ,"l");
  leg2->AddEntry(g_v_r60, "R_{4#pi#Lambda} = 60%" ,"l");
  leg2->AddEntry(g_v_r40, "R_{4#pi#Lambda} = 40%" ,"l");
  leg2->AddEntry(g_v_r20, "R_{4#pi#Lambda} = 20%" ,"l");
  */
  ...
  c->SaveAs("test_v_Lambda.pdf");
  c->SaveAs("test_v_Lambda.C");
  
  /*
  c->SaveAs("test_v_Lambda_4pi.pdf");
  c->SaveAs("test_v_Lambda_4pi.C");
  */
