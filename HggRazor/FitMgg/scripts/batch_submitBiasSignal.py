...
outputDir = "/afs/cern.ch/work/z/zhicaiz/public/BiasSignalTest_result/"
SoverB = ["1.0","3.0","5.0"]
...
if __name__ == "__main__":
	...
	os.system("mkdir -p "+pwd+"/submit_biasSignal")	
			...
			for SoverB_index in range(len(SoverB)):
				...
				for f1 in range(len(fitName)):
					for f2 in range(len(fitName)): 
						env_script_n = pwd + "/submit_biasSignal/" + bin_array[0]+"_"+bin_array[1]+"_"+bin_array[3]+"_SoB_"+SoverB[SoverB_index]+"_"+fitName[f1]+"_"+fitName[f2]+".sh"
						...
						submit_s = 'bsub -q '+queue+' -o ' + pwd + "/submit_biasSignal/"+bin_array[0]+"_"+bin_array[1]+"_"+bin_array[3]+"_SoB_"+SoverB[SoverB_index]+fitName[f1]+"_"+fitName[f2]+".log" + ' "source ' + env_script_n + '"'
