...
int main( int argc, char* argv[])
{
  ...
  std::map< std::pair <std::string, std::string>, double > mean_map;
  std::map< std::pair <std::string, std::string>, double > rms_map;
  if (  mode == "" )
    {
      std::cerr << "[ERROR]: please provide a mode by using --mode=<plot/table>" << std::endl;
      return -1;
    }
  ...
  if ( ifs.is_open() )
      ...
      while ( ifs.good() )
	{
	  ...
	  if ( mode == "plot" )
	    ...
	    {
	      std::pair<double, double> pair = GetMeanRms( biasFile, "plot_bias" );
	      //-----------------------------------
